/*
  ...
  ESP8266 + HC-SR04 + Custom Servo Pulse + Hub HTTP + Web UI + OTA
*/
...
#define FW_VERSION "5.0.0-HTTP-RC1"
...
/* ---------------- ROUTER / HUB HTTP CONFIG ---------------- */
static const char* WIFI_SSID = "CHANGE_ME";
static const char* WIFI_PASSWORD = "CHANGE_ME";
static const bool WIFI_USE_STATIC_IP = true;

// Set a reserved/static address for this sensor if desired.
static const IPAddress SENSOR_IP_1(192,168,0,20);
static const IPAddress SENSOR_IP_2(192,168,0,21);
static const IPAddress GATEWAY_IP(192,168,0,1);
static const IPAddress SUBNET_IP(255,255,255,0);
static const IPAddress DNS_IP(192,168,0,1);

static const IPAddress HUB_IP(192,168,0,10);
static const uint16_t HUB_PORT = 80;
...
const unsigned long SENSOR_FAULT_MS = 8000UL; // fast fault indication; local HTTP remains non-critical
...
struct Settings {
  ...
  uint16_t baselineDeltaCm; uint16_t heartbeatSec; uint16_t crowdUpdateSec; uint16_t reportIntervalMs;
};
Settings cfg;
...
unsigned long lastHttpReportAt = 0;
bool hubHttpHealthy = false;
...
void loadDefaults() {
  ...
  cfg.baselineDeltaCm = 35; cfg.heartbeatSec = 5; cfg.crowdUpdateSec = 2; cfg.reportIntervalMs = 1200;
}
...
void validateSettings() {
  ...
  if (cfg.reportIntervalMs < 500) cfg.reportIntervalMs = 500; if (cfg.reportIntervalMs > 10000) cfg.reportIntervalMs = 10000;
}
...
static void noteHttpResult(bool ok) {
  ...
  if (ok) {
    ...
    hubHttpHealthy = true;
    if (consecutiveSendOk < 250) consecutiveSendOk++;
    consecutiveSendFail = 0;
  } else {
    lastSendOk = false;
    hubHttpHealthy = false;
    if (consecutiveSendFail < 250) consecutiveSendFail++;
    consecutiveSendOk = 0;
  }
}

static bool sendHubJson(uint8_t eventType, uint32_t eventId) {
  if (WiFi.status() != WL_CONNECTED) { noteHttpResult(false); return false; }

  WiFiClient client;
  HTTPClient http;
  String url = String("http://") + HUB_IP.toString() + ":" + HUB_PORT + "/api/sensor";
  if (!http.begin(client, url)) { noteHttpResult(false); return false; }
  http.setTimeout(500);

  http.addHeader("Content-Type", "application/json");
  if (API_TOKEN[0] != '\0' && strcmp(API_TOKEN, "CHANGE_ME_LOCAL_TOKEN") != 0) {
    http.addHeader("X-Garur-Token", API_TOKEN);
  }

  char body[850];
  snprintf(body, sizeof(body),
    "{\"nodeId\":%u,\"nodeRole\":%u,\"eventType\":%u,\"presence\":%s,"
    "\"crowdLevel\":%u,\"zoneMask\":%u,\"targetAngle\":%u,\"targetDistanceCm\":%u,"
    "\"motionState\":%u,\"confidence\":%u,\"trackerState\":%u,\"servoAttached\":%u,"
    "\"activityScore\":%u,\"eventId\":%lu,\"uptimeMs\":%lu,\"sensorFault\":%s}",
    cfg.nodeId,cfg.nodeRole,eventType,detected?"true":"false",crowdLevel,zoneMask,targetAngle,targetDistanceCm,
    motionState,strongestConfidence,trackerState,servoSignalAttached?1:0,activityScore,(unsigned long)eventId,
    (unsigned long)millis(),sensorFault?"true":"false");
...
void queueEvent(uint8_t eventType, uint8_t repeats) {
  ...
  queuedRepeats = repeats;
  queuedEventId = eventSeq++;
  nextSendAt = 0;
}

void sendPacketNow(uint8_t eventType, uint32_t eventId) {
  sendHubJson(eventType, eventId);
}

void processHubReportQueue() {
  unsigned long now = millis();
  ...
  if (now - lastHeartbeatAt >= (unsigned long)cfg.heartbeatSec * 1000UL) {
    lastHeartbeatAt = now;
    sendPacketNow(EVT_HEARTBEAT, eventSeq++);
  }

  bool important = detected || sensorFault || (lastSendResultAt == 0);
  if (important && now - lastHttpReportAt >= cfg.reportIntervalMs) {
    lastHttpReportAt = now;
    uint8_t evt = sensorFault ? EVT_NODE_ERROR :
                  (detected ? EVT_CROWD_UPDATE : EVT_HEARTBEAT);
    sendPacketNow(evt, eventSeq++);
  }

  if (detected && now - lastCrowdUpdateAt >= (unsigned long)cfg.crowdUpdateSec * 1000UL) {
    lastCrowdUpdateAt = now;
    if (cfg.nodeRole == ROLE_EXIT_PATH && motionState == MOTION_STANDING &&
        presenceStartedAt > 0 &&
        ((now - presenceStartedAt) / 1000UL >= cfg.blockedTimeSec)) {
      sendPacketNow(EVT_PATH_BLOCKED, eventSeq++);
    } else if (cfg.nodeRole == ROLE_EXIT_PATH) {
      sendPacketNow(EVT_PATH_ACTIVITY, eventSeq++);
    } else {
      sendPacketNow(EVT_CROWD_UPDATE, eventSeq++);
    }
  }
}
...
void handleRoot() {
  ...
  sc(F("<p>Detection LED: <b id='detectLed'>-</b></p><p>Triggers: <b id='trig'>-</b></p><p>Hub HTTP OK/Fail: <b id='okfail'>-</b></p><p>Uptime: <b id='up'>-</b> s</p></div>"));
  ...
  sc(F("<script>"
       ...
       "document.getElementById('hubLed').innerHTML=j.hubLedState.includes('STABLE')?'<span class=ok>'+j.hubLedState+'</span>':(j.hubLedState.includes('UNSTABLE')?'<span class=warn>'+j.hubLedState+'</span>':'<span class=bad>'+j.hubLedState+'</span>');"
...
void handleSettings() {
  ...
  scRange("HTTP report interval", "reportIntervalMs", "reportIntervalMs", 500, 10000, 100, cfg.reportIntervalMs, " ms");
...
static bool apiJsonInt(const String& body, const char* key, long& out) {
  String n = String("\"") + key + "\"";
  int p = body.indexOf(n); if (p < 0) return false;
  p = body.indexOf(':', p + n.length()); if (p < 0) return false;
  p++; while (p < (int)body.length() && !isDigit(body[p])) p++;
  int e=p; while(e<(int)body.length() && isDigit(body[e])) e++;
  if(e==p) return false; out=body.substring(p,e).toInt(); return true;
}

void handleApiConfig() {
  if (API_TOKEN[0] != '\0' && strcmp(API_TOKEN, "CHANGE_ME_LOCAL_TOKEN") != 0) {
    if (!server.hasHeader("X-Garur-Token") || server.header("X-Garur-Token") != API_TOKEN) {
  ...
  long v;
  if (apiJsonInt(body,"detectCm",v)) cfg.detectCm=v;
  if (apiJsonInt(body,"clearCm",v)) cfg.clearCm=v;
  if (apiJsonInt(body,"detectStableMs",v)) cfg.detectStableMs=v;
  if (apiJsonInt(body,"clearStableMs",v)) cfg.clearStableMs=v;
  if (apiJsonInt(body,"sampleMs",v)) cfg.sampleMs=v;
  if (apiJsonInt(body,"servoStartAngle",v)) cfg.servoStartAngle=v;
  if (apiJsonInt(body,"servoEndAngle",v)) cfg.servoEndAngle=v;
  if (apiJsonInt(body,"servoStepDeg",v)) cfg.servoStepDeg=v;
  if (apiJsonInt(body,"servoSpeedMs",v)) cfg.servoSpeedMs=v;
  if (apiJsonInt(body,"servoIdleSec",v)) cfg.servoIdleSec=v;
  if (apiJsonInt(body,"miniScanIntervalSec",v)) cfg.miniScanIntervalSec=v;
  if (apiJsonInt(body,"trackingWindowDeg",v)) cfg.trackingWindowDeg=v;
  if (apiJsonInt(body,"fullRescanIntervalSec",v)) cfg.fullRescanIntervalSec=v;
  if (apiJsonInt(body,"mediumAngleCount",v)) cfg.mediumAngleCount=v;
  if (apiJsonInt(body,"highAngleCount",v)) cfg.highAngleCount=v;
  if (apiJsonInt(body,"standingTimeSec",v)) cfg.standingTimeSec=v;
  if (apiJsonInt(body,"blockedTimeSec",v)) cfg.blockedTimeSec=v;
  if (apiJsonInt(body,"baselineDeltaCm",v)) cfg.baselineDeltaCm=v;
  if (apiJsonInt(body,"heartbeatSec",v)) cfg.heartbeatSec=v;
  if (apiJsonInt(body,"crowdUpdateSec",v)) cfg.crowdUpdateSec=v;
  if (apiJsonInt(body,"reportIntervalMs",v)) cfg.reportIntervalMs=v;
...
void handleSave() {
  ...
  if (server.hasArg("reportIntervalMs")) cfg.reportIntervalMs = server.arg("reportIntervalMs").toInt();
...
void setupRouterWiFi() {
  WiFi.persistent(false);
  WiFi.setSleepMode(WIFI_NONE_SLEEP);
  WiFi.mode(WIFI_STA);
  if (WIFI_USE_STATIC_IP) {
    IPAddress local = (cfg.nodeId == 2) ? SENSOR_IP_2 : SENSOR_IP_1;
    WiFi.config(local, GATEWAY_IP, SUBNET_IP, DNS_IP);
  }
  WiFi.hostname((String("GarurSensor-") + String(cfg.nodeId)).c_str());
  ...
  if (WiFi.status() != WL_CONNECTED) {
    // Maintenance-only AP. Detection still runs; hub reporting resumes once Wi-Fi returns.
    String ssid = "GarurSensor-" + String(cfg.nodeId) + "-SETUP";
    WiFi.mode(WIFI_AP_STA);
    WiFi.softAP(ssid.c_str(), AP_PASSWORD, 6, false, 2);
  }
}
...
void setup() {
  ...
  loadSettings(); rebuildScanAngles(); setupRouterWiFi(); setupWebRoutes();
...
void loop() {
  if (WiFi.getMode() != WIFI_AP && WiFi.status() != WL_CONNECTED) {
    ...
    if (millis() - lastWifiTry >= 15000UL) { lastWifiTry = millis(); WiFi.begin(WIFI_SSID, WIFI_PASSWORD); }
  }
  server.handleClient(); processServoPulse(); processTracker();
  processHubReportQueue(); updateStatusLeds(); delay(2);
}